...
// node_modules/@x402/core/dist/esm/chunk-HX5GESJI.mjs
...
// node_modules/@x402/core/dist/esm/chunk-3LURPWBI.mjs
...
// _js/story-api-x402-browser-core.mjs
var BASE_SEPOLIA = {
  chainId: 84532,
  chainIdHex: "0x14a34",
...
function decodeChallenge(header) {
  if (!header) {
    throw new Error("Die 402-Antwort enth\xE4lt keinen PAYMENT-REQUIRED-Header.");
  }
  return decodePaymentRequiredHeader(header);
}
function selectAndValidateRequirement(paymentRequired, expected) {
  if (!paymentRequired || paymentRequired.x402Version !== 2 || !Array.isArray(paymentRequired.accepts)) {
    throw new Error("Ung\xFCltige x402-v2-Zahlungsanforderung.");
  }
  ...
  if (!paymentRequired.resource || typeof paymentRequired.resource.url !== "string") {
    throw new Error("Die Zahlungsanforderung enth\xE4lt keine g\xFCltige Ressource.");
  }
  if (!Number.isInteger(requirement.maxTimeoutSeconds) || requirement.maxTimeoutSeconds <= 0 || requirement.maxTimeoutSeconds > 60) {
    throw new Error("Sicherheitsstopp: Die G\xFCltigkeitsdauer muss zwischen 1 und 60 Sekunden liegen.");
  }
  if (!requirement.extra || typeof requirement.extra.name !== "string" || typeof requirement.extra.version !== "string") {
    throw new Error("Die EIP-712-Domain-Parameter f\xFCr USDC fehlen.");
  }
  return requirement;
}
function createEip3009TypedData(requirement, account, nowSeconds, nonce) {
...
async function createPaymentSignature(provider, account, paymentRequired, requirement, nowSeconds = Math.floor(Date.now() / 1e3), nonce = randomNonce()) {
  ...
  if (typeof signature !== "string" || !/^0x[0-9a-fA-F]+$/.test(signature)) {
    throw new Error("MetaMask hat keine g\xFCltige EIP-712-Signatur zur\xFCckgegeben.");
  }
  return {
    x402Version: 2,
...
// _js/story-api-x402-browser-test.js
var root = document.querySelector("#x402-browser-test");
if (root) {
...
async function connectWallet(state, controls) {
  ...
  try {
    ...
    if (!Array.isArray(accounts) || !accounts.length) {
      throw new Error("MetaMask hat kein K\xE4uferkonto freigegeben.");
    }
    ...
    setResult(controls, "Wallet verbunden. W\xE4hle das K\xE4uferkonto und pr\xFCfe das Netzwerk.");
  } catch (error) {
...
async function signAndFetch(state, controls, endpoint) {
  ...
  try {
    ...
    setResult(controls, "Zahlung best\xE4tigt und Vorlese-JSON freigeschaltet.");
  } catch (error) {
    setResult(controls, error.message, true);
  } finally {
    updatePayControl(state, controls);
...
function populateAccounts(controls, accounts) {
  controls.accounts.replaceChildren(new Option("Testk\xE4uferkonto w\xE4hlen", ""));
